...
for folder_idx in range(len(folders)):
    ...
    for PB_syl_idx in range(len(playback_syl)):
        [unique_g_PB_g, counts_g_PB_g] = np.unique(re.findall('Pg{,10}', re.sub('g+'+playback_syl[PB_syl_idx], 'P', bouts)),
                                                   return_counts=True)
        x_axis = [len(i) for i in unique_g_PB_g]
        ax1.plot(x_axis, counts_g_PB_g/np.sum(counts_g_PB_g), 'o-', color=colours[PB_syl_idx], label=playback_syl[PB_syl_idx]+'g')
