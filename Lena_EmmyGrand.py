...
colors_9 = ["#ff595e", "#ff924c", "#ffca3a", "#c5ca30", "#8ac926", "#52a675", "#1982c4", "#4267ac", "#6a4c93"]
...
for plot_idx in range(len(playback_list)):
    ...
    ax1.plot(bins[1:-1], counts[1:] / np.sum(counts[1:]), label=labels[plot_idx], linewidth=4, color=colors_rep[plot_idx+2])
...
for plot_idx in range(len(playback_list)):
    counts, bins = np.histogram(playback_list[plot_idx], steps)
    ax1.plot(bins[5:10], counts[5:10] / np.sum(counts[1:]), label=labels[plot_idx], linewidth=4, color=colors_rep[plot_idx+2])
ax1.set_xlabel('Repeat number')
ax1.set_ylabel('Rel. frequency')
plt.legend()
