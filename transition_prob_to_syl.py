...
def calc_transition_matrix(folder_path, folder, target_syl, trans_syl, transition_func):
    ...
    for folder_idx, folder_name in enumerate(folder):
        bouts = get_data(folder_path + folder_name + '*cbin.not.mat', ['E', 'S'], target_syl)
...
def main(folder_path, folder, target_syl, trans_syl, labels, transition):
    ...
    matrix = calc_transition_matrix(folder_path, folder, target_syl, trans_syl, transition_func)
    ...
    for trans_syl_idx, trans_syl_label in enumerate(trans_syl):
        label = f'{trans_syl_label}{target_syl}' if transition == 'converg' else f'{target_syl}{trans_syl_label}'
        ax1.plot(range(len(folder)), matrix_prob[:, trans_syl_idx], label=label, linewidth=4)
...
if __name__ == '__main__':
    ...
    main(path, folders, target_syllables[1][0], target_syllables[0], labels, transition_type)
